...
- **Terminal foreground precision.** `spawnTerminal` runs over E2B's native pty. Its output, input, resize, and kill are real, but the backend exposes no foreground process-group or input-waiting facts, so `inspectForeground` reports the session pid and `inputWaiting: false`. Interactive-shell heuristics that depend on precise foreground detection are weaker than the local backend.
